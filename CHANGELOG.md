...
### Changed
- Promoted the latest hot-path optimization release with synchronized metadata:
  - Keep `combined_output()` clone-avoidance in the runner/watch hot loop
  - Align release tag, changelog, and local install versioning

## [1.0.2] - 2026-04-09

### Changed
- Optimized `combined_output()` to avoid allocations in the hot watch loop:
  - Use `Cow<str>` to borrow stdout/stderr when only one stream has content
  - Eliminates unnecessary string clones for common cases (stderr-only or stdout-only)
  - Pre-sized allocation when both streams are present
